...
@section('custom-javascript')
    ...
    {{ Html::script("assets/js/lesson/track.js") }}
@endsection
                        ...
                        <div class="clearfix">
                            <h1>
                                @if ($lesson->completed === true)
                                    <i class="lesson-completed-icon-title fa fa-check-circle"></i>
                                @else
                                    <i id="lesson-completed-icon-title-{{ $lesson->id }}" class="lesson-completed-icon-title fa fa-check-circle d-none"></i>
                                    <i id="lesson-uncompleted-icon-title-{{ $lesson->id }}" class="lesson-uncompleted-icon-title fa fa-check-circle-o"></i>
                                @endif
                                {{ $lesson->title }}
                            </h1>
                        </div>
                        ...
                        <div class="tabs tabs--style-2" role="tabpanel">
                            <ul class="nav nav-tabs" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <a href="#tab-theory" aria-controls="home" role="tab" data-toggle="tab"
                                       class="nav-link active text-center text-normal strong-600">
                                        <i class="fa fa-book" aria-hidden="true"></i> Teoría
                                    </a>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <a href="#tab-comments" aria-controls="profile" role="tab" data-toggle="tab"
                                       class="nav-link text-center text-normal strong-600">
                                        ...
                                        @if (!isset($user) || (isset($user) && !$user['premium']))
                                            <div class="cta-block cta-block-overlay bg-base-1 z-depth-5 text-center">
                                                ...
                                                <a href="{{ route('pricing') }}"
                                                   class="btn btn-styled btn-lg btn-white btn-outline btn-circle mt-5">
                                                    Pásate a Premium
                                                </a>
                                            </div>
                                        @else
